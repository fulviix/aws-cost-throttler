...
describe("loadConfig", () => {
  ...
  it("should throw confignotfounderror if yaml doesnt exists", () => {
    expect(() =>
      loadConfig(join(testYAMLdir, "not-existing-file.yaml")),
    ).toThrow(ConfigNotFoundError);
  });
  it("should throw confignotfounderror if yaml syntax is malformed", () => {
    expect(() => loadConfig(join(testYAMLdir, "malformed.yaml"))).toThrow(
      ConfigNotFoundError,
    );
  });
  it("should throw configvalidationerror if yaml file is wrongly configurated", () => {
    expect(() => loadConfig(join(testYAMLdir, "invalidSchema.yaml"))).toThrow(
      ConfigValidationError,
    );
  });
  it("includes all errors found in ConfigValidationError.issues", () => {
    try {
      ...
      const paths = validationError.issues.map((issue) => issue.path.join("."));
